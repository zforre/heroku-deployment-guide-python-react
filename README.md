...
8. Add a database
	...
	* Add server-side environment variables to the Config Vars in Heroku.
		* Server-side env variables are stored in an .env file located in the root directory of your Django po
	...
	* Do not add client-side environment variables to the Config Vars section in Heroku.
		* Client-side env variables located inside an .env file located in the root directory of your React project on bundled when you run npm run build	
		* React env variables are exposed in your code
		* example React env variable: REACT_APP_BASE_URL = "sleepy-eyrie-95680.herokuapp.com"
...
10. Things to check if you have deployment issues:
	...
	* Make sure the frontend app includes:
		...
		* urls.py that renders the IndexView on the home route, `path('', IndexView.as_view(), name='index'),`
```
import os
import logging
